...
import {
  ...
  formatTime,
  getPrice
} from "../display/utility-functions";
...
export class TemplateComponent {
  ...
  constructor(
    private celSvr: ChumiElementLoadService,
    ...
    this.http
      .getDomainRunningActivitiesSimple("www.chumi.co")
      .subscribe(data => {
        console.log("getDomainRunningActivitiesSimple: ", data);
      });

    this.http.getMyRunningActivitiesSimple().subscribe(data => {
    ...
    this.http.getPageData(this.domain, encodeURIComponent(this.path)).subscribe(
      data => {
        ...
        if (previewMode) {
          ...
          this.data["chumi_footer"] =
            data["sharedData"]["data"]["chumi_footer"];
          this.data["chumi_header"] =
            data["sharedData"]["data"]["chumi_header"];
        } else {
          this.data = this.getDefaultData(this.getPageName());
          // go to 404 page
        }
  ...
  getDefaultData(page: string) {
    switch (page) {
      ...
      case "default":
        return DefaultTemplateData.default;
    }
  }
  ...
  add() {
    ...
    this.tile.addEventListener("pageReady", $event =>
      console.log("viewReady = ", $event)
    );
  ...
  addToolBarToElement(element, editFunc, className) {
    if (element) {
      ...
      if (
        !element.classList.contains("chumi_slogan") &&
        !element.classList.contains("chumi_header")
      ) {
        button = document.createElement("button");
